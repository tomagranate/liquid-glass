...
try {
  for (const file of ["dist/index.js", "dist/index.cjs", "dist/index.d.ts"]) {
    ...
    if (/\bfrom\s*["']react(?:\/|["'])|\brequire\(["']react(?:\/|["'])/.test(emitted)) {
...
`,
  ...
  run("npm", ["install", "--ignore-scripts", "--no-package-lock", tarball], vanilla);
...
`,
  ...
  run("npm", ["install", "--ignore-scripts", "--no-package-lock", tarball, "react@18", "react-dom@18"], react);
